...
emoji_flood_progress = 0


def _increment_flood_meter(socketio, sender_clean: str, emoji_clean: str = "🎉") -> None:
    global emoji_flood_progress
    emoji_flood_progress += 2.5  # ~40 emoji taps to fill the 100% meter
    if emoji_flood_progress >= 100:
        emoji_flood_progress = 0
        socketio.emit("flood_meter_update", {"progress": 100, "triggered": True, "sender": sender_clean})
        socketio.emit("emoji_flood", {"emoji": emoji_clean, "sender": sender_clean})
    else:
        socketio.emit("flood_meter_update", {"progress": emoji_flood_progress, "triggered": False})
...
def setup_socket_events(socketio):
    ...
    def handle_send_emoji(data: dict) -> None:
        """Handle floating emoji reaction from a guest client (allowed anytime)."""
        emoji = str(data.get("emoji", "")).strip() if isinstance(data, dict) else ""
        sender = str(data.get("sender", "Guest")).strip() if isinstance(data, dict) else "Guest"
        if sender and sender.strip().lower() in ("andres", "andy"):
            sender = "SkyDancer"
        avatar = str(data.get("avatar", "🎤")).strip() if isinstance(data, dict) else "🎤"
        if not emoji:
            return
        sender_clean = html.escape(sender[:30])
        emoji_clean = html.escape(emoji[:10])
        avatar_clean = html.escape(avatar[:10])
        socketio.emit("emoji_reaction", {"emoji": emoji_clean, "sender": sender_clean, "avatar": avatar_clean})
        _increment_flood_meter(socketio, sender_clean, emoji_clean)

    @socketio.on("send_shoutout")
    def handle_send_shoutout(data: dict) -> None:
        """Handle custom shoutout text message from a guest client."""
        ...
        if not message:
            return
        sender_clean = html.escape(sender[:30])
        message_clean = html.escape(message[:120])
        avatar_clean = html.escape(avatar[:10])
        socketio.emit("shoutout", {"message": message_clean, "sender": sender_clean, "avatar": avatar_clean})
        _increment_flood_meter(socketio, sender_clean, "💬")

    @socketio.on("request_flood_meter")
    def handle_request_flood_meter() -> None:
        """Client requests current flood meter progress."""
        socketio.emit("flood_meter_update", {"progress": emoji_flood_progress, "triggered": False}, room=request.sid)
